...
autAppleRL <-  "https://api.twitter.com/oauth/authorize"
consumerKey <-  "##############################"
consumerSecret <-  "############################"
...
my_oauth <- OAuthFactory$new(consumerKey=consumerKey,
                             ...
                             accessURL=accessURL, autAppleRL=autAppleRL)
...
tweets<-searchTwitter('#Apple', n =500, lang="en", since='2014-03-01', until='2018-12-06')
...
#Apple tweets analyzing

#Analyze the followers of Apple
##What language do Apple friends speak?###################################
user <- getUser("tim_cook")
friends <- user$getFriends() # who Apple follows
friends_df <- twListToDF(friends)
friends_df$lang  ##What language do Apple friends speak?

# what language do they speak?
unique(friends_df$lang)
#save(friends_df, file = "Apple_friends.RData")

#followers <- user$getFollowers() # Apple followers
#followers_df <- twListToDF(followers)
#save(followers_df, file = "Apple_followers.RData")
...
##On average, Apple friends tweet 6.8 times a day in the past year.
...
#Among Apple friends, the person with most friends is Kumar Garg; 
...
#What are the most commonly used words in Apple friends followers' descriptions? (Tidytext package)###########################
...
ggplot(wscount_t10, aes(x = reorder(word, count), y = count)) +
  ...
  theme_light() +
  coord_flip() +
  labs(x = "", y = "count of words in Apple friend's description")

#The Top 10 most talked about words by Apple friends are 
#news, pennlive, PA, harrisburg, reporter, media, central, science, patriot and community.



##Optional: Are Apple followers talk positively or negatively abut Apple?#############################
...
barplot(df1$value, names = df1$sentiment,
        ...
        main = "Sentiment analysis for #Apple ",
